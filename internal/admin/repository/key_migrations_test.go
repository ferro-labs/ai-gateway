...
func TestCreatedKeyIsNeverWrittenInClear(t *testing.T) {
	...
	created, err := store.Create(context.Background(), "fresh", []string{model.ScopeAdmin}, nil)
	if err != nil {
...
VALUES('empty-id', ?, '...', 'empty', '["admin"]', ?, NULL, NULL, NULL, 1, 0, NULL)`
	...
	mem.byID["empty-id"] = &keyRecord{apiKey: &model.APIKey{ID: "empty-id", Active: true}, hash: hashKey("")}
...
func TestWriteAheadLogInheritsRestrictedMode(t *testing.T) {
	...
	if _, err := store.Create(context.Background(), "wal", []string{model.ScopeAdmin}, nil); err != nil {
		t.Fatalf("create key: %v", err)
	}
