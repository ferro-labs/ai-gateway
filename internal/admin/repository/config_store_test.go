...
CREATE TABLE gateway_config (
	...
	if loaded.Strategy.Mode != config.ModeFallback {
		t.Fatalf("legacy config mode = %q, want %q", loaded.Strategy.Mode, config.ModeFallback)
	}
...
func (s *failingConfigStore) Save(_ context.Context, cfg config.Config) error {
	if s.saveErr != nil {
		return s.saveErr
	}
...
func (s *failingConfigStore) Load(context.Context) (config.Config, bool, error) {
...
func TestGatewayConfigManager_ReloadConfig_ClassifiesValidationErrors(t *testing.T) {
	...
	invalid := config.Config{
		Strategy: config.StrategyConfig{Mode: "invalid"},
		Targets:  []config.Target{{VirtualKey: "openai"}},
	}
...
func TestGatewayConfigManager_NilStore(t *testing.T) {
	...
	if mgr.GetConfig().Strategy.Mode != config.ModeSingle {
...
func (s *successConfigStore) Save(_ context.Context, c config.Config) error { s.cfg = c; return nil }
func (s *successConfigStore) Load(context.Context) (config.Config, bool, error) {
	return s.cfg, s.cfg.Strategy.Mode != "", nil
}
func (s *successConfigStore) Delete(context.Context) error { s.cfg = config.Config{}; return nil }
...
func TestGatewayConfigManager_WithPersistedConfig(t *testing.T) {
	...
	if mgr.GetConfig().Strategy.Mode != config.ModeFallback {
		t.Errorf("expected persisted mode %q to be applied, got %q", config.ModeFallback, mgr.GetConfig().Strategy.Mode)
	}
}
