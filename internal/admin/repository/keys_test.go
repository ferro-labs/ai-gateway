...
// TestCreateScopeDefault pins what an unspecified scope list grants. A caller
// that names no scope has not asked for admin, so the key it gets must not be
// able to mint further keys. Both stores are asserted because a deployment's
// privileges must not depend on which storage backend it happens to run.
func TestCreateScopeDefault(t *testing.T) {
	cases := []struct {
		name   string
		scopes []string
		want   []string
	}{
		{name: "nil scopes", scopes: nil, want: []string{model.ScopeReadOnly}},
		{name: "empty scopes", scopes: []string{}, want: []string{model.ScopeReadOnly}},
		{name: "explicit read-only", scopes: []string{model.ScopeReadOnly}, want: []string{model.ScopeReadOnly}},
		{name: "explicit admin", scopes: []string{model.ScopeAdmin}, want: []string{model.ScopeAdmin}},
		{name: "explicit multi-scope", scopes: []string{model.ScopeReadOnly, model.ScopeAdmin}, want: []string{model.ScopeReadOnly, model.ScopeAdmin}},
	}
		...
		t.Run(impl.name, func(t *testing.T) {
			for _, tc := range cases {
				t.Run(tc.name, func(t *testing.T) {
					...
					created := seedKey(t, store, tc.name, tc.scopes)

					if !slices.Equal(created.Scopes, tc.want) {
						t.Fatalf("created scopes = %v, want %v", created.Scopes, tc.want)
					}
					if got, want := model.HasAdminScope(created.Scopes), slices.Contains(tc.want, model.ScopeAdmin); got != want {
						t.Fatalf("admin scope granted = %v, want %v", got, want)
					}

					stored, ok := store.Get(t.Context(), created.ID)
					if !ok {
						t.Fatal("expected key to exist")
					}
					if !slices.Equal(stored.Scopes, tc.want) {
						t.Fatalf("stored scopes = %v, want %v", stored.Scopes, tc.want)
					}
				})
			}
		})
...
func TestGet_ReturnsDefensiveCopy(t *testing.T) {
	...
	created, _ := store.Create(context.Background(), "copy-me", []string{model.ScopeReadOnly}, &expiresAt)
	...
	*got.ExpiresAt = got.ExpiresAt.Add(time.Hour)
	...
	if again.Scopes[0] != model.ScopeReadOnly {
		t.Fatalf("stored scope = %q, want %q", again.Scopes[0], model.ScopeReadOnly)
	}
...
func TestValidateKey_ReturnsDefensiveCopy(t *testing.T) {
	store := NewKeyStore()
	created, _ := store.Create(context.Background(), "validate-copy", []string{model.ScopeReadOnly}, nil)
	...
	validated.Scopes[0] = model.ScopeAdmin
	...
	if stored.Scopes[0] != model.ScopeReadOnly {
		t.Fatalf("stored scope = %q, want %q", stored.Scopes[0], model.ScopeReadOnly)
	}
}
