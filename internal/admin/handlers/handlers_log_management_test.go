...
// dimension mirrors one group of an encoded stats dimension. Each group now
// carries its error and token totals, not only how many events it saw.
...
func TestLogsStatsEndpoint(t *testing.T) {
	reader := &fakeLogReader{stats: requestlog.StatsResult{
		...
		ErrorEntries: 1,
		TotalTokens:  35,
		ByStage:      map[string]requestlog.DimensionStat{"after_request": {Count: 2}, "on_error": {Count: 1}},
		ByProvider:   map[string]requestlog.DimensionStat{"openai": {Count: 2}, "anthropic": {Count: 1}},
		ByModel:      map[string]requestlog.DimensionStat{"gpt-4": {Count: 2}, "claude": {Count: 1}},
	}}
	h, r := setupTestRouterWithLogs(reader)
	adminKey := createAdminKey(t, h)
	...
	var payload struct {
		...
		ByProvider map[string]dimension `json:"by_provider"`
		ByModel    map[string]dimension `json:"by_model"`
	}
	...
	if payload.ByStage["after_request"].Count != 2 || payload.ByStage["on_error"].Count != 1 {
		t.Fatalf("unexpected by_stage: %+v", payload.ByStage)
	}
	if payload.ByProvider["openai"].Count != 2 || payload.ByProvider["anthropic"].Count != 1 {
		t.Fatalf("unexpected by_provider: %+v", payload.ByProvider)
	}
	if payload.ByModel["gpt-4"].Count != 2 || payload.ByModel["claude"].Count != 1 {
...
func TestLogsStatsEndpointWithLimit(t *testing.T) {
	...
	if len(payload.ByProvider) != 1 || payload.ByProvider["openai"].Count != 2 {
		t.Fatalf("unexpected limited by_provider: %+v", payload.ByProvider)
	}
	if len(payload.ByModel) != 1 || payload.ByModel["gpt-4"].Count != 2 {
...
func TestLogsStatsEndpointReturnsExactCounts(t *testing.T) {
	...
	reader := &fakeLogReader{stats: requestlog.StatsResult{
		...
		ErrorEntries: 7,
		TotalTokens:  5010,
		ByStage:      map[string]requestlog.DimensionStat{"after_request": {Count: 5010}},
		ByProvider:   map[string]requestlog.DimensionStat{"openai": {Count: 5010}},
		ByModel:      map[string]requestlog.DimensionStat{"gpt-4": {Count: 5010}},
	}}
	h, r := setupTestRouterWithLogs(reader)
	adminKey := createAdminKey(t, h)
