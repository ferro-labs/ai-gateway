...
func TestCreateKeyWithScopes(t *testing.T) {
	...
	var created model.APIKey
	decodeJSON(t, w.Body, &created)
	if len(created.Scopes) != 1 || created.Scopes[0] != model.ScopeReadOnly {
...
func TestGetKeyByID(t *testing.T) {
	...
	if got.Key != repository.DisplayKey(created.Key) {
		t.Fatalf("expected the display key %q, got %q", repository.DisplayKey(created.Key), got.Key)
	}
}
