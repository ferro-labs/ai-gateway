...
func (s *dbFailKeyStore) RotateKey(context.Context, string) (*model.APIKey, error) {
...
func TestKeyHandler_StoreFailure_Returns500AndDoesNotLeak(t *testing.T) {
	h := &Handlers{Keys: &dbFailKeyStore{KeyStore: repository.NewKeyStore()}}
...
func TestKeyHandler_NotFound_StillReturns404(t *testing.T) {
	h := &Handlers{Keys: repository.NewKeyStore()}
	w := httptest.NewRecorder()
