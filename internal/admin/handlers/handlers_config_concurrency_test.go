...
func configMarker(cfg config.Config) float64 {
...
func TestGatewayConfigManagerPersistsWhatItApplies(t *testing.T) {
	...
	m, err := repository.NewGatewayConfigManager(gw, store)
	if err != nil {
...
func doAdminRequest(t *testing.T, r http.Handler, method, url, body string, key *model.APIKey, wantStatus int) {
	t.Helper()
...
func (m *latentConfigManager) GetConfig() config.Config {
	m.mu.Lock()
	defer m.mu.Unlock()
	return m.cfg
}

func (m *latentConfigManager) ReloadConfig(_ context.Context, cfg config.Config) error {
...
func (s *orderedConfigStore) Save(_ context.Context, cfg config.Config) error {
...
func (s *orderedConfigStore) record(cfg config.Config) {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.saved, s.hasCfg = cfg, true
}

func (s *orderedConfigStore) Load(_ context.Context) (config.Config, bool, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *orderedConfigStore) Delete(_ context.Context) error {
	s.mu.Lock()
	defer s.mu.Unlock()
	s.saved, s.hasCfg = config.Config{}, false
	return nil
}

func (s *orderedConfigStore) last() (config.Config, bool) {
	s.mu.Lock()
	defer s.mu.Unlock()
