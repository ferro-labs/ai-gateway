...
func TestStreamDecoder_CapturesUsageAcrossStartAndDelta(t *testing.T) {
	...
	// 25 fresh + 4 cache-read: PromptTokens is reported INCLUSIVE of the
	// cache-read subset (the OpenAI convention), so 29, and TotalTokens 44.
	if final.Usage.PromptTokens != 29 || final.Usage.CompletionTokens != 15 || final.Usage.TotalTokens != 44 {
		t.Fatalf("usage tokens = %d/%d/%d, want 29/15/44",
...
// A malformed message_delta frame must not emit a spurious terminal chunk with
// an empty finish reason and zeroed usage; a well-formed frame still emits its
// terminal chunk.
func TestStreamDecoder_MalformedMessageDeltaEmitsNoChunk(t *testing.T) {
	// delta is a string instead of an object, so the frame fails to decode.
	d := NewStreamDecoder("anthropic", "")
	chunks, err := d.Event([]byte(`{"type":"message_delta","delta":"end_turn","usage":{"output_tokens":9}}`))
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(chunks) != 0 {
		t.Fatalf("malformed message_delta emitted %d chunks, want 0: %#v", len(chunks), chunks)
	}

	// A well-formed frame on a fresh decoder still emits its terminal chunk.
	d2 := NewStreamDecoder("anthropic", "")
	good, err := d2.Event([]byte(`{"type":"message_delta","delta":{"stop_reason":"end_turn"},"usage":{"output_tokens":9}}`))
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(good) != 1 {
		t.Fatalf("well-formed message_delta emitted %d chunks, want 1: %#v", len(good), good)
	}
	if good[0].Choices[0].FinishReason != core.FinishReasonStop {
		t.Fatalf("finish = %q, want stop", good[0].Choices[0].FinishReason)
	}
	if good[0].Usage == nil || good[0].Usage.CompletionTokens != 9 {
		t.Fatalf("usage = %#v, want completion 9", good[0].Usage)
	}
}

// The streaming decoder is the second Anthropic-wire decode boundary, and it is
// separate code from the non-streaming Response. Usage reported on the final
// chunk must carry the same cache-inclusive PromptTokens the non-streaming path
// produces, so a request costs the same whether or not it was streamed.
func TestStreamDecoder_PromptTokensAreCacheInclusive(t *testing.T) {
	d := NewStreamDecoder("anthropic", "")
	chunks, err := drain(d,
		`{"type":"message_start","message":{"id":"m","model":"claude","usage":{"input_tokens":2000,"cache_read_input_tokens":8000,"cache_creation_input_tokens":600}}}`,
	...
	if u.PromptTokens != 10000 {
		t.Errorf("PromptTokens = %d, want 10000 (2000 fresh + 8000 cache read)", u.PromptTokens)
	}
	if u.TotalTokens != 10050 {
		t.Errorf("TotalTokens = %d, want 10050", u.TotalTokens)
	}
	if u.CacheReadTokens != 8000 || u.CacheWriteTokens != 600 {
		t.Errorf("cache tokens = %d/%d, want 8000/600", u.CacheReadTokens, u.CacheWriteTokens)
	}
}

// A stream with no cache hits must report exactly what it always did.
func TestStreamDecoder_PromptTokensUnchangedWithoutCache(t *testing.T) {
	d := NewStreamDecoder("anthropic", "")
	chunks, err := drain(d,
		`{"type":"message_start","message":{"id":"m","model":"claude","usage":{"input_tokens":2000}}}`,
		`{"type":"message_delta","delta":{"stop_reason":"end_turn"},"usage":{"output_tokens":50}}`,
	)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	...
	u := chunks[len(chunks)-1].Usage
	if u == nil || u.PromptTokens != 2000 || u.TotalTokens != 2050 || u.CacheReadTokens != 0 {
