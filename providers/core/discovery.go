// Model-discovery helpers for providers that support live model enumeration via
// OpenAI-compatible GET /v1/models (or similar) endpoints.
...
func DiscoverModelsWithHeaders(ctx context.Context, client *http.Client, url string, headers map[string]string, providerName string) ([]ModelInfo, error) {
	...
	body, err := ReadResponseBody(resp.Body, MaxProviderResponseBytes)
	if err != nil {
		return nil, fmt.Errorf("failed to read discovery response: %w", err)
	}

	if resp.StatusCode != http.StatusOK {
		// A refused redirect carries an empty or HTML body, from which
		// upstreamMessage recovers nothing — so the target host is the only
		// thing that makes "HTTP 302" actionable. It replaces the body message
		// rather than joining it for that reason.
		if hint := transport.RedirectTarget(resp); hint != "" {
			return nil, StatusError(providerName, resp.StatusCode, hint).WithRetryAfter(resp.Header)
		}
		return nil, APIErrorFromResponse(providerName, resp, body)
	}
