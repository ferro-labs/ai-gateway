...
func TestTogetherProvider_Transcribe_Interface(_ *testing.T) {
	p, _ := New("test-key", "")
	var _ core.TranscriptionProvider = p
}

func TestTogetherProvider_Transcribe_MockHTTP(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		// Together model ids are namespaced (openai/whisper-large-v3); forwarded verbatim.
		if r.FormValue("model") != "openai/whisper-large-v3" {
			t.Errorf("model = %q, want openai/whisper-large-v3", r.FormValue("model"))
		}
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"text":"hello together"}`))
	}))
	defer srv.Close()

	p, _ := New("test-key", srv.URL)
	resp, err := p.Transcribe(context.Background(), core.TranscriptionRequest{
		Model: "openai/whisper-large-v3", File: []byte("AUDIODATA"), Filename: "clip.mp3",
	})
	if err != nil {
		t.Fatalf("Transcribe() error: %v", err)
	}
	if resp.Text != "hello together" {
		t.Errorf("Text = %q, want 'hello together'", resp.Text)
	}
}

func TestTogetherProvider_Transcribe_TranslatePath(t *testing.T) {
	var gotPath string
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		gotPath = r.URL.Path
		w.Header().Set("Content-Type", "application/json")
