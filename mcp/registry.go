...
const (
	...
	// defaultToolCallTimeout is the per-tool-call timeout applied when a server
	// config specifies no (or an invalid) TimeoutSeconds.
...
type Registry struct {
	log *logger.Logger // injected; never nil after NewRegistry
	...
	// teardown tracks the goroutine Close detaches when requests still hold the
	// registry. Without it that goroutine is unreachable: Close returns nil
	// immediately, the caller's bounded drain has nothing to wait on, and the
	// process can exit with the stdio subprocess still running — the exact
	// orphan the drain exists to reap.
...
// NewRegistry creates an empty Registry. A nil log falls back to logger.Default().
	...
	if old, ok := r.servers[cfg.Name]; ok {
		...
		if old.client != nil {
			if err := old.client.Close(); err != nil {
				r.log.Warn("mcp: failed to close replaced server client",
...
func (r *Registry) markUnready(name string, client mcpClient, cause error) {
	...
	entry.ready = false
	entry.initErr = cause
	r.reindexToolsLocked(name, entry.tools)
	r.mu.Unlock()

	r.setServerUp(name, 0)
	r.log.Warn("mcp: server is no longer available; its tools are withdrawn",
		"server", name, "error", cause)
}

// reindexToolsLocked re-points each tool name the named server held at the
// first still-ready server that also exposes it. Caller holds r.mu.
//
// A withdrawal is not only the dead server's business: toolMap is the
// resolution index, so a name left pointing at it stays unresolvable even when a
// second server advertises the same tool and is answering. AllTools falls
// through to that server regardless, so without the re-index the model is told
// about a tool FindToolServer then refuses to resolve.
//
// Takeover walks registration order, the same first-registered-wins policy
// initServer indexes with, and consults each candidate's own tool list — already
// filtered by its AllowedTools — so no server inherits a name it was configured
// not to expose.
//
// A name no ready server can take keeps its mapping rather than being deleted.
// It does not resolve (FindToolServer checks the ready bit), and toolMap doubles
// as the set of names the registry has ever indexed, which is what keeps a real
// tool that stopped resolving from being collapsed into the unknown-tool metric
// label — see boundedToolLabel.
func (r *Registry) reindexToolsLocked(name string, tools []Tool) {
	for _, t := range tools {
		if r.toolMap[t.Name] != name {
			continue
		}
		for _, other := range r.regOrder {
			entry, ok := r.servers[other]
			if other == name || !ok || !entry.ready {
				continue
			}
			if slices.ContainsFunc(entry.tools, func(o Tool) bool { return o.Name == t.Name }) {
				r.toolMap[t.Name] = other
				break
			}
		}
	}
...
func (r *Registry) closeWhenDrained() {
	...
	if err := r.closeClients(); err != nil {
		r.log.Error("mcp: failed to close retired registry", "error", err)
	}
}
...
// WaitClosed blocks until a teardown Close detached has finished, or until ctx
// is done.
//
// Close deliberately returns immediately while requests still hold the registry,
// so its caller can bound the wait rather than block on a stdio server working
// through the graceful/SIGTERM/SIGKILL ladder. That bound needs something to
// wait ON, and this is it. It returns nil when teardown completed and ctx.Err()
// when the caller's budget ran out first — at which point the subprocess is
// still being terminated, and the caller has decided not to wait longer.
//
// Safe to call when no teardown was detached, and safe to call more than once.
func (r *Registry) WaitClosed(ctx context.Context) error {
