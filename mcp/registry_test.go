	...
	return httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		case mcpMethodInitialize:
			...
			_ = json.NewEncoder(w).Encode(JSONRPCResponse{
				...
				Result: initializeResult("mock", "1"),
			})
...
// A withdrawn server must not keep a tool name hostage. toolMap is the
// resolution index, so a name still pointing at a dead server is unresolvable
// even when a second, ready server advertises the same tool — while AllTools
// keeps offering it to the model.
func TestRegistryWithdrawnServerReleasesItsToolNames(t *testing.T) {
	primary, backup := &countingClient{}, &countingClient{}
	...
	for _, s := range []struct {
		...
		{"primary", primary, []Tool{{Name: "shared_tool"}, {Name: "solo_tool"}}},
		{"backup", backup, []Tool{{Name: "shared_tool"}}},
	} {
		reg.serverIndex[s.name] = len(reg.regOrder)
		reg.regOrder = append(reg.regOrder, s.name)
		reg.servers[s.name] = &serverEntry{
	...
	// First-registered-wins: primary holds both names while it is alive.
	reg.toolMap["shared_tool"] = "primary"
	reg.toolMap["solo_tool"] = "primary"

	reg.markUnready("primary", primary, errors.New("transport gone"))

	client, ok := reg.FindToolServer("shared_tool")
	if !ok {
		t.Fatal("shared_tool stopped resolving although a ready server still exposes it")
	}
	if client != mcpClient(backup) {
		t.Error("shared_tool still resolves to the withdrawn server")
	}
	// Nothing can take solo_tool, so it stays unresolvable — the name is kept
